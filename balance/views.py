...
def get_commodity_data(request):
    ...
    for commodity in commodity_list:
        ...
        box = box['boxes__sum']
        begs_out = gate_pass.objects.filter(Q(Name=partyName) & Q(commodity=commodity[0])).aggregate(Sum('bags'))
        begs_out = begs_out['bags__sum']
        boxes_out = gate_pass.objects.filter(Q(Name=partyName) & Q(commodity=commodity[0])).aggregate(Sum('boxes'))
        boxes_out = boxes_out['boxes__sum']
